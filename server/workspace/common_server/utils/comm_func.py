...
def get_service_host(s_ip, s_port_dic):
    return ":".join([s_ip] + [str(port) for port in sorted(s_port_dic.values())])
...
def move_file(src_file_path, tgt_file_name, tgt_file_path):
    """
    linux文件移动
    ...
    if platform.system() == 'Linux':
        subprocess.Popen("mv %s %s/%s" % (src_file_path, tgt_file_path, tgt_file_name), shell=True)
        logger.info("move_file move_file from:%s to:%s/%s" % (src_file_path, tgt_file_path, tgt_file_name))